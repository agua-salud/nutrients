...
source("assets/captioner/captions/captions_filtering.R")
source("assets/reactable/download_this_fun.R")
source("assets/reactable/styles.R")
source("assets/reactable/table_functions/filtering.R")
```
...
<small>`r caption_tab_ssu("ssu_filtering_results")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-body"}
download_this_fun(ssu_filtering_results)
```

```{r, echo=FALSE, layout="l-body", eval=TRUE}
filter_summary(seq_table)
rm(seq_table)
```
...
<small>`r caption_tab_ssu("ssu_filter_sample_data")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-page"}
ssu_filter_sample_data <- ssu_all_filt_samp_summary
download_this_fun(ssu_filter_sample_data)
```

```{r, echo=FALSE, layout="l-page", eval=TRUE}
filter_full_table(seq_table)
rm(seq_table)
```
...
<small>`r caption_tab_its("its_filtering_results")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-body"}
download_this_fun(its_filtering_results)
```
...
<small>`r caption_tab_its("its_filter_sample_data")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-page"}
its_filter_sample_data <- its_all_filt_samp_summary
download_this_fun(ssu_filter_sample_data)
```
...
```{r eval=TRUE, echo=FALSE, layout="l-body"}
ssu_arbitrary_filt_summary <- ssu_sum_arbitrary
download_this_fun(ssu_arbitrary_filt_summary)
```

```{r, echo=FALSE, layout="l-body", eval=TRUE}
filter_results_summ(seq_table)
rm(seq_table)
```
...
<small>`r caption_tab_ssu("ssu_arbitrary_filt_samples")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-page"}
ssu_arbitrary_filt_samples <- ssu_filt_samp_summary
download_this_fun(ssu_arbitrary_filt_samples)
```

```{r, echo=FALSE, layout="l-page", eval=TRUE}
filter_results_sample(seq_table)
rm(seq_table)
```
...
```{r eval=TRUE, echo=FALSE, layout="l-body"}
its_arbitrary_filt_summary <- its_sum_arbitrary
download_this_fun(its_arbitrary_filt_summary)
```
...
<small>`r caption_tab_its("its_arbitrary_filt_samples")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-page"}
its_arbitrary_filt_samples <- its_filt_samp_summary
download_this_fun(its_arbitrary_filt_samples)
```
...
```{r eval=TRUE, echo=FALSE, layout="l-body"}
ssu_perfect_filt_summary <- ssu_sum_perfect
download_this_fun(ssu_perfect_filt_summary)
```
...
```{r eval=TRUE, echo=FALSE, layout="l-body"}
its_perfect_filt_summary <- its_sum_perfect
download_this_fun(its_perfect_filt_summary)
```
...
<small>`r caption_tab_ssu("ssu_perfect_filt_samples")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-page"}
ssu_perfect_filt_samples <- ssu_perfect_samp_summary
download_this_fun(ssu_perfect_filt_samples)
```
...
<small>`r caption_tab_its("its_perfect_filt_samples")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-page"}
its_perfect_filt_samples <- its_perfect_samp_summary
download_this_fun(its_perfect_filt_samples)
```
...
```{r eval=TRUE, echo=FALSE, layout="l-body"}
ssu_pime_oob_best_prev <- ssu_best.prev$`OOB error`
download_this_fun(ssu_pime_oob_best_prev)
```

```{r, echo=FALSE, layout="l-body", eval=TRUE}
pime_oob_best_prev(seq_table)
rm(seq_table)
```
...
ssu_pime_filt_top_asvs <- ssu_imp_best
download_this_fun(ssu_pime_filt_top_asvs)
```

```{r, echo=FALSE, layout="l-body-outset", eval=TRUE}
seq_table <- seq_table %>% 
 mutate_if(is.numeric, round, digits = 4)
pime_best_prev_summ(seq_table)
rm(seq_table)
```
...
```{r eval=TRUE, echo=FALSE, layout="l-body"}
ssu_pime_filt_summary <- ssu_sum_pime
download_this_fun(ssu_pime_filt_summary)
```
...
<small>`r caption_tab_ssu("ssu_pime_filt_samples")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-page"}
ssu_pime_filt_samples <- ssu_pime_samp_summary
download_this_fun(ssu_pime_filt_samples)
```
...
<small>`r caption_tab_ssu("ssu_pime_change_by_year_treat")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-body"}
ssu_pime_change_by_year_treat <- ssu_asv_pime_sum
download_this_fun(ssu_pime_change_by_year_treat)
```

```{r, echo=FALSE, layout="l-body", eval=TRUE}
pime_changes_by_yr_treat(seq_table)
rm(seq_table)
```
...
```{r eval=TRUE, echo=FALSE, layout="l-body"}
its_pime_oob_best_prev <- its_best.prev$`OOB error`
download_this_fun(its_pime_oob_best_prev)
```
...
```{r eval=TRUE, echo=FALSE, layout="l-body-outset"}
its_pime_filt_top_asvs <- its_imp_best
download_this_fun(its_pime_filt_top_asvs)
```
...
```{r eval=TRUE, echo=FALSE, layout="l-body"}
its_pime_filt_summary <- its_sum_pime
download_this_fun(its_pime_filt_summary)
```
...
<small>`r caption_tab_its("its_pime_filt_samples")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-page"}
its_pime_filt_samples <- its_pime_samp_summary
download_this_fun(its_pime_filt_samples)
```
...
<small>`r caption_tab_its("its_pime_change_by_year_treat")`</small>

```{r eval=TRUE, echo=FALSE, layout="l-body"}
its_pime_change_by_year_treat <- its_asv_pime_sum
download_this_fun(its_pime_change_by_year_treat)
```
